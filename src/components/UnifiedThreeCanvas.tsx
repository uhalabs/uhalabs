...
function ScrollDrivenCamera({ currentPath }: { currentPath: string }) {
  ...
  useEffect(() => {
    ...
    if (currentPath === "/platform") target = { x: 160, y: 220, z: 440, tx: 0, ty: 60, tz: 0 };
      ...
      target = { x: 0, y: 0, z: -800, tx: 0, ty: 0, tz: -1500 };
    else if (currentPath === "/solutions")
      target = { x: 360, y: -360, z: 230, tx: 0, ty: -600, tz: 200 };
    else if (currentPath === "/industries")
      target = { x: -270, y: -540, z: 540, tx: 0, ty: -600, tz: 200 };
    else if (currentPath === "/about") target = { x: 420, y: 220, z: 440, tx: 0, ty: 0, tz: 0 };
    else if (currentPath === "/contact")
      target = { x: 0, y: -1050, z: 440, tx: 0, ty: -600, tz: 200 };

    gsapRef.current?.kill();
    gsapRef.current = gsap.to(cameraControl.current, {
      ...target,
      duration: 2.2,
      ease: "power2.inOut",
    });
  ...
  useFrame(() => {
    ...
    if (currentPath === "/") {
      ...
      smoothScrollRef.current +=
        (Math.max(0, Math.min(1, targetSp)) - smoothScrollRef.current) * 0.07;
    ...
    () =>
      Array.from({ length: 12 }).map((_, i) => {
        ...
        return {
          x: Math.cos(a) * r,
          z: Math.sin(a) * r,
          color: i % 3 === 0 ? "#7c3aed" : i % 3 === 1 ? "#06b6d4" : "#10b981",
        };
      }),
    [],
  );
...
function NeuralTunnel() {
  ...
  const tunnelGeo = useMemo(
    () => new THREE.EdgesGeometry(new THREE.CylinderGeometry(160, 160, 1300, 24, 24, true)),
    [],
  );
  const innerGeo = useMemo(
    () => new THREE.EdgesGeometry(new THREE.CylinderGeometry(90, 90, 1300, 16, 16, true)),
    [],
  );
...
function Scene({ currentPath }: { currentPath: string }) {
  ...
  useFrame(() => {
    ...
    if (cmdRef.current) cmdRef.current.visible = currentPath !== "/" || (sp > 0.5 && sp < 0.84);
    if (agentRef.current) agentRef.current.visible = currentPath !== "/" || (sp > 0.7 && sp < 0.95);
...
export function UnifiedThreeCanvas({ currentPath }: UnifiedThreeCanvasProps) {
  ...
  return (
    <div
      className="fixed inset-0 h-screen w-screen pointer-events-none -z-20"
    ...
    >
      ...
      >
        ...
        <PerformanceMonitor onDecline={() => setDpr(1)} onIncline={() => setDpr(2)} flipflops={2} />
